module Fastlane
  module Actions
    ...
    class UpdatePlistFileAction < Action
      def self.run(params)
        ...
        [
          ...
          FastlaneCore::ConfigItem.new(key: :plist_path,
                                       ...
                                       verify_block: proc do |value|
                                          ...
                                          UI.user_error!("Invalid plist file") unless (value[-6..-1].casecmp(".plist").zero? || value[-13..-1].casecmp(".entitlements").zero?)
                                       end),
      ...
      def self.example_code
        [
          'update_plist_file( # update app identifier string
            plist_path: "path/to/Info.plist",
            app_identifier: "com.example.newappidentifier"
          )',
          'update_plist_file( # Change the Display Name of your app
            plist_path: "path/to/Info.plist",
            display_name: "MyApp-Beta"
          )',
          'update_plist_file( # Target a specific `xcodeproj` rather than finding the first available one
            xcodeproj: "path/to/Example.proj",
            plist_path: "path/to/Info.plist",
            display_name: "MyApp-Beta"
          )',
          'update_plist_file( # Advanced processing: find URL scheme for particular key and replace value
            xcodeproj: "path/to/Example.proj",
            plist_path: "path/to/Info.plist",
            block: proc do |plist|
